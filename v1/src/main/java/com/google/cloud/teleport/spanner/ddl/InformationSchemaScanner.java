...
public class InformationSchemaScanner {
  ...
  private void listIndexColumns(
    ...
    while (resultSet.next()) {
      ...
      String indexName = resultSet.getString(4);
  ...
  private void listSequenceOptionsGoogleSQL(
    ...
    while (resultSet.next()) {
      ...
      if (Objects.equals(optionName, Sequence.SEQUENCE_START_WITH_COUNTER)
    ...
    for (Map.Entry<String, Long> entry : currentCounters.entrySet()) {
      ...
      options.add(
          Sequence.SEQUENCE_START_WITH_COUNTER + "=" + newCounterStartValue);
      LOG.info(
          "Sequence "
              + entry.getKey()
